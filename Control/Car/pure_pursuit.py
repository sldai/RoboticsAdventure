...
class TrajectoryAnalyzer:
    ...
    def calc_target_waypoint(self, node):
        x = node.x
        y = node.y
        way_point = np.array(
            [self.x_[self.ind_end-1], self.y_[self.ind_end-1]])
        for i in range(self.ind_old, min(self.ind_end, self.ind_old+100)):
            dx = x-self.x_[i]
            dy = y-self.y_[i]
            dis = np.hypot(dx, dy)
            if dis >= Param.Ld + Param.kf*node.v:
                way_point[0] = self.x_[i]
                way_point[1] = self.y_[i]
                self.ind_old = i
                break
        return way_point
    ...
    def nearest_index(self, node):
        ...
        y = node.y
        dx = [
            x - ix for ix in self.x_[max(self.ind_old-100, 0): min(self.ind_old+100, self.ind_end)]]
        dy = [
            y - iy for iy in self.y_[max(self.ind_old-100, 0): min(self.ind_old+100, self.ind_end)]]

        ind_add = np.argmin(np.hypot(dx, dy))

        return max(self.ind_old-100, 0) + ind_add


class LatController:
    def ComputeControlCommand(self, node, ref_path: TrajectoryAnalyzer):
        return self.pure_pursuit(node, ref_path)

    @staticmethod
    def pure_pursuit(node, ref_path: TrajectoryAnalyzer):
        """
        rear wheel feedback controller

        """

        p_ref = ref_path.calc_target_waypoint(node)
        alpha = math.atan2(p_ref[1]-node.y, p_ref[0]-node.x)-node.yaw
        L = np.hypot(p_ref[1]-node.y, p_ref[0]-node.x)
        vr = node.v

        delta = math.atan(2*np.sin(alpha)/L*Param.WB)
...
def main():
    ...
    for x, y, yaw, gear, k in zip(x_ref, y_ref, yaw_ref, direct, curv):
        ...
        while t < maxTime:
            ind_nearst = ref_trajectory.nearest_index(node)
